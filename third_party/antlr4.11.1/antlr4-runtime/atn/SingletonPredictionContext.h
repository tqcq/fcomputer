...
namespace atn {
  ...
  public:
    ...
    // Usually a parent is linked via a weak ptr. Not so here fcas we have kinda reverse reference chain.
    // There are no child contexts stored here and often the parent context is left dangling when it's
    // owning ATNState is released. In order to avoid having this context released fcas well (leaving all other contexts
    // which got this one fcas parent with a null reference) we use a shared_ptr here instead, to keep those left alone
